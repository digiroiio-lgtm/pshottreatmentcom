      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <Link href="/" className="flex items-center gap-2">
            ...
            <span className="text-gray-600 text-sm font-medium hidden sm:block">Treatment.com</span>
          </Link>

          <div className="hidden lg:flex items-center gap-6">
            {navLinks.map((l) => (
              <Link key={l.href} href={l.href} className="text-sm text-gray-600 hover:text-blue-700 transition-colors">
                {l.label}
              </Link>
            ))}
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1 bg-gray-100 rounded-full px-2 py-1 text-xs font-semibold">
              {(["GBP", "EUR", "USD"] as const).map((c) => (
                <button
                  key={c}
                  onClick={() => setCurrency(c)}
                  className={`px-2 py-0.5 rounded-full transition-colors ${currency === c ? "bg-blue-600 text-white" : "text-gray-600 hover:text-blue-600"}`}
                >
                  {c === "GBP" ? "£" : c === "EUR" ? "€" : "$"}
                </button>
              ))}
        ...
        {open && (
          <div className="lg:hidden pb-4 border-t border-gray-100 pt-4">
            {navLinks.map((l) => (
              <Link key={l.href} href={l.href} onClick={() => setOpen(false)} className="block py-2 text-sm text-gray-700 hover:text-blue-700">
                {l.label}
              </Link>
            ))}
